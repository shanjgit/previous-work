    ...
    def averageLinkageDist(self, other):
        ...
        for p in self.points:
            for q in other.points:
                dis = p.distance(q)
                disList.append(dis)
        avg = sum(disList)/float(len(disList))
        return avg
    def members(self):
        for p in self.points:
            yield p
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        if c1.pointType != c2.pointType:
            raise ValueError('c1.pointType should equal to c2.pointType')
        tempPoints = []
        tempC1 = c1.points[:]
        tempC2 = c2.points[:]
        typeC = c1.pointType
    ...
    def findClosest(self, linkage):
        ...
        minDistance = linkage(self.members[0], self.members[1])
        toMerge = (self.members[0], self.members[1])
        for c1 in self.members:
            for c2 in self.members:
                if c1 == c2:
                    continue
                if linkage(c1, c2) < minDistance:
                    minDistance = linkage(c1, c2)
                    toMerge = (c1, c2)
        return toMerge        
